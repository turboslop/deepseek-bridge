...
Use `skills/issue-to-green-pr/SKILL.md` when picking up a GitHub issue or implementation task in this repository. It defines the expected workflow: update `main`, create a `codex/` branch, implement narrowly, run tests and linters, open a PR, watch CI, fix failures until checks are green, and rely on auto-merge.
