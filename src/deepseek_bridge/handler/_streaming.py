...
from ..logging import LOG, log_json
...
class HandlerStreaming:
    def _proxy_streaming_response(
        ...
        try:
            ...
            while True:
                ...
                if (
                    alive_check_counter % 10 == 0
                    and not self._check_client_alive()
                ):
                    LOG.debug(
                        "handler.disconnect: client disconnected at "
                        "stage=streaming"
                    )
                ...
                try:
                    ...
                    if isinstance(exc, urllib3.exceptions.ReadTimeoutError):
                        LOG.warning(
                            "upstream streaming response read timed out "
                            "after %ss: %s",
                ...
                if finalized:
                    ...
                    line.lstrip().startswith(b"data:") and b"[DONE]" not in line
                ):
            ...
            if not finalized:
                ...
                if self.config.debug and stored:
                    LOG.info(
                        "stored %s streaming reasoning cache key(s) before "
                        "exit",
                        stored,
                    )
    ...
    def _rewrite_sse_line(
        ...
        if data == b"[DONE]":
            ...
            stored = accumulator.flush_pending_store(self.reasoning_store)
            scope_preview = (
                response_contexts[0][0] if response_contexts else ""
            )[:16]
            LOG.debug(
