...
class RecoveryTests(_StrictUpstreamCase):
    def test_cold_cache_patches_missing_reasoning(self) -> None:
        """Stale tool history with no cached reasoning: proxy patches
        missing reasoning with a placeholder instead of dropping context."""
        ...
        sent = StrictFakeDeepSeek.requests[-1]
        # All messages are preserved (patch strategy does not drop context)
        self.assertEqual(
            [m["role"] for m in sent["messages"]],
            ["system", "user", "assistant", "tool", "user"],
        )
        self.assertEqual(
            sent["messages"][2]["reasoning_content"], "[reasoning recovered]"
        )
        self.assertEqual(
            sent["messages"][-1]["content"], "Thanks. What about Saturday?"
        )
        # No recovery notice in the response
        self.assertNotIn(
            "[deepseek-bridge]",
            response["choices"][0]["message"]["content"],
        )

    def test_echoed_assistant_content_not_stripped(self) -> None:
        """When Cursor echoes a previous response back as assistant content,
        strip_recovery_notice_for_upstream must not corrupt regular content
        (no recovery notice prefix is generated by the patch strategy)."""
        # Prime the cache so the echoed assistant has stored reasoning.
        status, first = _post(
            f"{self.proxy.url}/v1/chat/completions",
            {
                "model": "deepseek-v4-pro",
        ...
        # Cursor echoes the response (no notice prefix with patch strategy).
    ...
    def test_recover_mode_does_not_short_circuit_with_409(self) -> None:
        """In `recover` mode, a payload with no user message still gets
        patched (patch strategy fills missing reasoning with a placeholder)
        and forwarded upstream. 409 is reserved for `reject` mode."""
        status, _ = _post(
            f"{self.proxy.url}/v1/chat/completions",
            {
                "model": "deepseek-v4-pro",
        ...
        # The patch strategy fills missing reasoning, so upstream accepts the
        # request. The point of this test is the proxy did NOT pre-empt with 409.
